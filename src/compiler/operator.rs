        ...
        + AddAssign,
{
    pub fn run(
        ...
        match self {
            ...
            Self::Qwen36FullAttention(operator) => {
                operator.run(
                    position_index,
                    position_interval,
                    batch_size,
...
fn test_operator_matmul_f16_dispatch_and_parallel_consistency() {
    ...
    let matmul1 = unsafe {
        crate::compiler::mul::matmul::MatMul::<f16>::new(
            a.as_ptr(),
            b_nt.as_ptr(), // ✅ 传 NT
            c1.as_mut_ptr(),
            false,
    ...
    op1.run(position_index, position_interval, batch_size, 1, 0);
    ...
    for tid in 0..thread_num {
        op2.run(position_index, position_interval, batch_size, thread_num, tid);
    }
...
fn test_operator_matmul_topk_f16_avx512_dispatch_small_no_ties() {
    ...
    unsafe {
        let thread_max = crate::compiler::mul::matmul_topk::MatMulTopK::<f16>::detect_threads();
        let buf_len = M * thread_max * TOPK;
        let mut indices_buf = vec![0usize; buf_len];
        let mut values_buf = vec![0.0f16; buf_len];

        let runner = crate::compiler::mul::matmul_topk::MatMulTopK::<f16>::new(
            a.as_ptr(),
            b_nt.as_ptr(), // ✅ 传 NT
...
fn test_operator_experts_matmul_silu_f16_dispatch_single_expert_basic() {
    ...
    unsafe {
        let runner = crate::compiler::mul::experts_matmul_silu_mul_matmul::ExpertsMatMulSilu::<f16>::new(
            a.as_ptr(),
...
fn test_operator_experts_down_f16_dispatch_basic_mb_gt_mr() {
    ...
    for ex in 0..e {
        for bb in 0..b {
            for kk in 0..hmid {
                nonlin[(ex * b + bb) * hmid + kk] = (0.001f32 * (ex as f32)
                    + 0.01f32 * (bb as f32)
                    + 0.0003f32 * (kk as f32)) as f16;
            }
        }
    }
    ...
    unsafe {
        let runner = crate::compiler::mul::experts_matmul_mul::ExpertsMatMulDown::<f16>::new(
            nonlin.as_ptr(),
            wdown_nt.as_ptr(), // ✅ NT
            experts_indicator.as_ptr(),
            indice.as_ptr(),
...
fn test_operator_experts_down_f16_dispatch_tail_cols() {
    ...
    for bb in 0..b {
        for kk in 0..hmid {
            nonlin[bb * hmid + kk] =
                (0.01f32 * (bb as f32) + 0.0003f32 * (kk as f32)) as f16;
        }
    }
...
fn assert_close(got: f32, exp: f32, tol: f32, msg: &str) {
    let diff = (got - exp).abs();
    if diff > tol {
        panic!("{} got={}, exp={}, diff={}, tol={}", msg, got, exp, diff, tol);
    }
}
...
fn test_operator_merge_add_f16_dispatch_k1_basic() {
    ...
    unsafe {
        let runner = crate::compiler::mul::experts_merge_add::ExpertsMergeAdd::<f16>::new(
            input.as_ptr(),
            residual.as_ptr(),
            experts_indicator.as_mut_ptr(),
            indice_ptr.as_mut_ptr(),
...
fn test_operator_matmul_add_f16_dispatch_and_parallel_consistency() {
    ...
    let mut c1 = vec![0.0f16; M * N];
    let runner1 = unsafe {
        crate::compiler::mul::matmul_add::MatMulAdd::<f16>::new(
            a.as_ptr(),
            b_nt.as_ptr(),        // ✅ 传 NT
